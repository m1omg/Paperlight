...
(function (P) {
  ...
  function answer(m, state) {
    ...
    if (/\b(turn off|switch off|disable|hide|mute|block) (the )?chat\b|\bchat off\b|\bstrangers? (can'?t|cannot|can) (message|talk to|chat with)\b/.test(text) && /\b(minecraft|server|servers|online|multiplayer|realms?|him|her|my (son|daughter|kid|child))\b/.test(text)) { const g = D.guides.find((x) => x.q.includes("can i turn off the chat in minecraft")); if (g) return done(state, { text: g.a, kind: "guide" }, null); }
