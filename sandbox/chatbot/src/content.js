...
(function (P) {
  ...
  const intents = [
    ...
    { id: "good_night", ex: ["good night", "night night", "gn", "going to sleep", "i am going to bed", "time for bed", "i need to sleep", "sleep well", "nighty night", "i am tired i am going to sleep"],
      re: /\b(good ?night|night night|nighty night|going to (sleep|bed)|going to go (to )?sleep|off to (sleep|bed)|time for bed|gotta sleep|go to sleep now|heading to bed|gn)\b|(?<!\b(every|last|all|at|each|one|the|a|that|this|same|whole|monday|tuesday|wednesday|thursday|friday|saturday|sunday|movie|game|date|pizza|sleepover|first|second|other|school|work|late|opening|family|fun|good|bad|long|busy|tonight's|tomorrow)\s)\b(good ?)?(night|nite)( night)?( everyone| pip| all)?[!. ]*$/,
    ...
    { id: "bye", ex: ["bye", "goodbye", "see you", "see ya", "later", "gotta go", "i have to go", "talk to you later", "ttyl", "cya", "bye bye", "i am leaving", "i will be back", "see you tomorrow", "catch you later", "peace out", "farewell", "i got to go"],
      re: /^(bye+|goodbye|good bye|see (you|ya)|cya|later|ttyl|gotta go|got to go|i have to go|i gotta go|bye bye|peace( out)?|farewell|adios|see you (later|soon|tomorrow))\b|\b(i am going to|gonna|going to|about to|i am about to) head out\b|\b(i am|im) (out|off|heading out|logging off)[.! ]*$|\b(logging off|gotta bounce|gotta dip|i am gonna dip)\b|\b(bye+|goodbye|bye bye|see (you|ya)( later| soon| tomorrow)?|cya|ttyl|gtg|g2g|later|peace out|gotta go|have to go now)( pip| now| for now| everyone)?[.! ]*$/,
    ...
    { id: "trivia", ex: ["quiz me", "trivia", "play trivia", "give me a quiz", "trivia question", "test my knowledge"], re: /^(?!.*\b(what|who|which|where|when|how)\b.*\b(do i|am i|did i|my|me)\b)(?:.*\b(quiz me|trivia|(give|ask) me a quiz|(play|do|start|have) (a )?quiz|test my knowledge|quiz (time|game|night))\b|\s*(a )?quiz[!.?]*$)/, say: (c) => c.skill("trivia") },
  ...
  const advice = [
    ...
    { re: /\b(forget (my|the) (line|lines|words)|stage fright|laugh on stage|remember my (line|lines)|learn my (line|lines))\b|\b(nervous|scared|worried)\b.*\b(play|show|performance|stage|line|lines)\b/, need: /./,
    ...
    { re: /\b(what (do|should|can) i (tell|say to)|how do i (tell|explain)|(don'?t|do not|dont) know what to (tell|say to))\b.*\b(him|her|them|my (little |younger |baby |big )?(brother|sister|siblings?))\b|\b\w+ (keeps|kept) asking\b.*\b(dad|mom|mum|coming (back|home)|divorce)\b/, need: /./,
    ...
    { re: /\b(divorce|divorced|splitting up|separat\w*|my parents fight|two houses|switch houses)\b/, need: /./,
